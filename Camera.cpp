...
void CCamera::Slide_Camera(float speed)
{
	tVector3 vVector = mView - mPos;	// Get the view vector
...
void CCamera::Mouse_Move_1P(int wndWidth, int wndHeight)
{
	POINT mousePos;	
	int mid_x = wndWidth  >> 1;	
	int mid_y = wndHeight >> 1;	
...
tVector3 CCamera::GetViewMove(float speed)
{
	tVector3 vVector = mView - mPos;	// Get the view vector
	tVector3 view;
	
	// forward positive cameraspeed and backward negative -cameraspeed.
	view.x = mView.x + vVector.x * speed;
	view.z = mView.z + vVector.z * speed;

	return view;
}

tVector3 CCamera::GetViewSlide(float speed)
{
	tVector3 vVector = mView - mPos;	// Get the view vector
	tVector3 view;
	...
	{
	// forward positive camera speed and backward negative camera speed.
		view.x = mView.x + vVector.z * speed;
		view.z = mView.z - vVector.x * speed;
	}
	else if((mView.x < mPos.x) && (mView.z > mPos.z))
	{
	// forward positive camera speed and backward negative camera speed.
