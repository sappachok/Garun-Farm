#ifndef EN_BOSS05_CPP
#define EN_BOSS05_CPP
#endif
...
void CBoss05::Load()
{
	CMD2Model::Load("Data\\Model\\goblin\\tris.md2","Data\\Model\\goblin\\boss05.tga",0.1f);
	weapon.Load("Data\\Model\\goblin\\weapon.md2","Data\\Model\\goblin\\weapon.tga",0.1f);
...
void CBoss05::OnDraw(CCamera *camera)
{
...
void CBoss05::OnCollision(CObject *collisionObject) 
{ 	
...
void CBoss05::ProcessAI()
{
	...
	if((aiState != AI_DEAD) && (aiState != AI_PAIN) && (aiState != AI_ATTACK) &&
		(aiState != AI_ANGRY) && (((CPlayer*)player)->LifePoint>0))
	{
		if(distFromPlayer < 0)
		{
			aiState = AI_SCARED;
		}
		else if(distFromPlayer < size + target->size)
		{
			aiState = AI_ATTACK;
		}
		else
	...
	if((aiState != AI_DEAD) && (aiState != AI_PAIN)  && (aiState != AI_ATTACK) && 
		(target->type == MD_FRIEND) && (aiState != AI_ANGRY))
	{
...
void CBoss05::OnAnimate(float deltaTime)
{
	...
	switch(aiState)
	{
		...
		case AI_ATTACK:
			...
			if((m_nextFrame == 50) && (canAttack == true))
			{
				...
				CSuperSonic *sonic = new CSuperSonic;
				sonic->AttachTo(place);
				sonic->Create(position.x, 0, position.z);	
				sonic->direction = DEG2RAD(direction);
				sonic->rotate = -direction;
		...
		case AI_DEAD:
			...
			if(m_nextFrame == m_startFrame)
			{
				...
				for(int i=0; i<10; i++)
				{
					...
					int x = (int)(rand()%20-10);
					int z = (int)(rand()%20-10);
					...
					if(type <= 3)
					{
						...
						reddiamond->Load();
						reddiamond->CreateItem(position.x+x, 0, position.z+z);	
					}
					else if(type<=8)
					{
						...
						bluediamond->Load();
						bluediamond->CreateItem(position.x+x, 0, position.z+z);	
					}
					else if(type<10)
					{
...
void CBoss05::OnCollisionBox(CObject *collisionObject)
{
