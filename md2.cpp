...
void CMD2Model::AnimateSet(CMD2Model *md2Model)
{
  m_state = md2Model->m_state;
  m_startFrame = md2Model->m_startFrame;
  m_currentFrame = md2Model->m_currentFrame;
  m_nextFrame = md2Model->m_nextFrame;
  m_endFrame = md2Model->m_endFrame;
  m_interpol = md2Model->m_interpol;
}
...
void CMD2Model::DrawBoundingBox()
{
/*	int numofFrame = m_currentFrame;
	...
	glColor3f(0.0f, 1.0f, 0.0f);
		glVertex3f( m_boundingBox[numofFrame].min.x , m_boundingBox[numofFrame].min.y , m_boundingBox[numofFrame].min.z);
		glVertex3f( m_boundingBox[numofFrame].max.x , m_boundingBox[numofFrame].min.y , m_boundingBox[numofFrame].min.z);
		glVertex3f( m_boundingBox[numofFrame].max.x , m_boundingBox[numofFrame].min.y , m_boundingBox[numofFrame].max.z);
		glVertex3f( m_boundingBox[numofFrame].min.x , m_boundingBox[numofFrame].min.y , m_boundingBox[numofFrame].max.z);
	glEnd();

	glBegin(GL_LINE_LOOP);
	glColor3f(0.0f, 1.0f, 0.0f);
		glVertex3f( m_boundingBox[numofFrame].min.x , m_boundingBox[numofFrame].max.y , m_boundingBox[numofFrame].min.z);
		glVertex3f( m_boundingBox[numofFrame].max.x , m_boundingBox[numofFrame].max.y , m_boundingBox[numofFrame].min.z);
		glVertex3f( m_boundingBox[numofFrame].max.x , m_boundingBox[numofFrame].max.y , m_boundingBox[numofFrame].max.z);
		glVertex3f( m_boundingBox[numofFrame].min.x , m_boundingBox[numofFrame].max.y , m_boundingBox[numofFrame].max.z);
	glEnd();
...
/*
tBounding CMD2Model::GetBoundingBox() 
{ 
	return m_boundingBox[m_currentFrame]; 
}

int CMD2Model::CheckCollisionBox(tBounding b_target)
{
	tBounding m_newBounding;
	m_newBounding.max.x = m_boundingBox[m_currentFrame].max.x+m_pos.x;
	m_newBounding.min.x = m_boundingBox[m_currentFrame].min.x+m_pos.x;
	m_newBounding.max.y = m_boundingBox[m_currentFrame].max.y+m_pos.y;
	m_newBounding.min.y = m_boundingBox[m_currentFrame].min.y+m_pos.y;
	m_newBounding.max.z = m_boundingBox[m_currentFrame].max.z+m_pos.z;
	m_newBounding.min.z = m_boundingBox[m_currentFrame].min.z+m_pos.z;

	if( m_newBounding.max.x < b_target.min.x ) 
		return 0;
	if( m_newBounding.min.x > b_target.min.x )
		return 0;
	if( m_newBounding.max.x < b_target.max.x ) 
		return 0;
	if( m_newBounding.min.x > b_target.max.x )
		return 0;

	if( m_newBounding.max.z < b_target.min.z ) 
		return 0;
	if( m_newBounding.min.z > b_target.min.z )
		return 0;
	if( m_newBounding.max.z < b_target.max.z ) 
		return 0;
	if( m_newBounding.min.z > b_target.max.z )
		return 0;
