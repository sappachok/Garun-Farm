...
public:
  ...
  void AnimateSet(CMD2Model *md2Model);
  ...
  int GetFrameAnimate() { return m_currentFrame; }

  int GetModelState() { return m_state; }

  const sphere_t &GetBoundingSphere() { return m_boundingSphere; }

//  int CheckCollisionBox(tBounding b_target);

  void Move(GLfloat x, GLfloat y, GLfloat z);
  void Scale(GLfloat x, GLfloat y, GLfloat z) { m_scaleX = x; m_scaleY = y; m_scaleZ = z; };
  void Rotate(GLfloat angle) { m_rotate = angle; }
//  tBounding GetBoundingBox();
